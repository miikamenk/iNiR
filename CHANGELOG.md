...
Fork-local changes on top of upstream. Not versioned — `VERSION` tracks upstream.

### Added
- **Liquid style**: 6th global style — subtle liquid glass with gentle blur, translucent surfaces, a light-from-above edge highlight, sheen, and soft diffused shadows. Implemented as an aurora superset (`Appearance.liquid.*`, config-tunable under `appearance.liquid.*`), so it inherits every glass code path and adds its own tokens at the unified dispatch points. Pick it in Settings → Themes (ii and waffle), or the welcome wizard.
- **Clock dashboard**: left-click the bar clock for a large animated clock (seconds ticking, no global service abuse), full calendar, merged upcoming events with the event editor, and weather — current conditions, next-24h hourly strip, and 7-day forecast with temperature range bars. Also via IPC `clockDashboard toggle`.
- **System dashboard**: left-click the bar resources indicator for CPU (with per-core mini-bars), RAM, swap, GPU, VRAM, network rx/tx with history graph, disk usage + I/O rates, and temperatures. Cards carry 60-sample history graphs. Also via IPC `systemDashboard toggle`.
- **Resizable sidebars**: drag the inner edge of either sidebar to resize (320–860 px), persisted via `sidebar.widthLeft`/`sidebar.widthRight` (plus `sidebar.widthExtended` for the expanded left sidebar). Settings → Interface gets matching spinboxes. Both sidebars, both directions, live preview while dragging.
- **Weather forecast data**: the weather service now exposes `forecast` (7 days) and `hourly` (next 24h) from both providers, plus `tempUnit`. Open-Meteo WMO codes are normalized to the wttr code space, so the fallback provider finally shows correct icons and descriptions.
- **ResourceUsage extensions**: VRAM (nvidia-smi and AMD/Intel sysfs), network rx/tx rates with history, per-core CPU percentages, disk I/O rates, all behind the existing lazy-polling lifecycle. `formatRate()` helper included.
- **Niri visual refresh**: crafted per-interaction animation springs (window open overshoots slightly, close is a clean 150ms fade, movement/resize are critically damped), themed focus ring enabled by default with a primary→tertiary gradient from the wallpaper palette, softer window shadows, inactive window dim relaxed to 0.95, corner radius matched to the shell. Migration 031 applies it to uncustomized configs.
- **Dashboards slide out of their bar widget**: the clock and system dashboards are now anchored popups that emerge from behind the bar edge and stay next to the widget that owns them, instead of appearing centred over the screen. New shared `BarAnchoredPanel` widget (the ii counterpart to waffle's `BarPopup`) handles anchoring, the slide animation, hover/pin state, click-outside and Escape. Works for both the horizontal and vertical bar, on either screen edge.
- **Hover to peek, click to pin**: hovering the bar clock or resources widget reveals its dashboard after a short dwell (350 ms, so sweeping along the bar doesn't fling a panel open); it retracts once the pointer leaves both the widget and the panel. Clicking pins it open until you click it again, click outside, or press Escape. Pinning is still `GlobalStates`-backed, so IPC and keybinds drive the same panel.
- **Calendar day selection**: pressing a day now selects it — a ring marks the active day and the weather panel retargets to that date, showing its high/low and rain chance. Pressing the day that is *already* active is what opens its detail page. A single press no longer yanks you off the month grid toward an event form. The matching forecast row is highlighted so the link is obvious.
- **Real transparency for panels**: liquid style now gives its surfaces genuine alpha instead of painting a blurred copy of the wallpaper, so windows behind a panel actually show through. Applies to both sidebars (all three layouts), the clock and system dashboards, the control panel, the bar, the vertical bar, and the bar media popup. Content drawn on top keeps full opacity, so text stays crisp over arbitrary window content. Toggle and opacity slider under Settings → Themes → Liquid Style Editor (`appearance.liquid.realGlass`).
- **Specular gleam**: a soft diagonal band of light raked across glass surfaces — the "wet" part of liquid glass. Configurable (`appearance.liquid.specular`).
- **Refraction rim**: a hairline inset border on all four edges, following the surface's corner radius. A see-through panel otherwise fades into whatever is behind it with no defined edge.
- **Liquid Style Editor** in Settings → Themes: real transparency, glass opacity, specular gleam and strength, edge highlight and strength, sheen.
- **Solid herdr chrome on a translucent terminal**: herdr draws its sidebar by leaving the terminal's default background showing, and kitty applies `background_opacity` only to default-background cells — so a see-through terminal made herdr's chrome see-through too, with no herdr-side setting to change it. The generated kitty theme now splits the two: default-background cells (herdr's sidebar and tab bar, painted in `panel_bg`) sit at 0.98, while `transparent_background_colors` carries the opacity from `kitty.conf` for the pane color. Pane shells claim that color for themselves with an `OSC 11` on startup (`inir-terminal-bg.fish` plus a sourced POSIX snippet for zsh/bash), which is what moves them out of the default-background bucket. The rule lists the last 7 pane colors, so panes opened before a wallpaper change stay translucent until their shell restarts, and a kitty window with no herdr in it gets its opacity back through `kitten @`. Only active when herdr is installed and `background_opacity` is below 1.
- **herdr follows the wallpaper**: a `herdr/config.toml` template renders the terminal workspace manager's chrome from the palette — `panel_bg` is the Material background (the same color the shell's panels and the terminal background come from), the `surface_dim`/`surface0`/`surface1` ladder is the elevation ladder, and the named colors map by role rather than by hue (`accent` → primary, `red` → error, `green` → success, `yellow`/`peach` → the warning family, `mauve` → the tone-shifted accent). herdr has no include mechanism, so the template carries the non-theme settings too; a running server is reloaded via `herdr server reload-config` from the render post-hook, so the colors change with the wallpaper without a restart.
- **hyprglass follows the wallpaper**: the Hyprland glass plugin's window tint used to be a hardcoded blue-grey, so themed windows clashed with the shell sitting on top of them. A new `hyprglass-colors.lua` template renders the palette tokens the shell's liquid glass is built from, and `custom/plugins.lua` reproduces the shell's own recipe on them — wallpaper seed blended toward `primary_container`, mixed into the Material background, painted at `appearance.liquid.realGlass.opacity`. Both themes get their own tint (`dark`/`light`), applied globally and in the `apple` preset. hyprglass composites its tint exactly as a QML surface does (`mix(scene, tint, alpha)`), so window glass and panel glass now land on the same colour. Falls back to the old static tint if the generated file is missing or half-written.

### Changed
- **Color pipeline balance pass**: every generated palette (shell + templates) now gets surface chroma caps (no more muddy tinted surfaces from vibrant wallpapers), an accent chroma floor for muted seeds (respecting monochrome/neutral intent), tertiary harmonization toward the primary, and a palette-derived success family instead of hardcoded greens.
- **Unified app theming**: `build_app_palette()` now emits HCT-stepped `app_accent_hover/active/subtle`, a stronger selection with border, and semantic `app_error/success/warning/info` families. Vesktop statuses are actually semantic now (online = green, idle = amber — was lavender and blue, because reasons), Spicetify's active button uses the accent step instead of a recycled selection color, and the Neovim plugin spec needs way less saturation pumping.
- Bar module spacing is tokenized (`Appearance.sizes.barModuleSpacing`/`barPillPadding`) and symmetric between left and right sections. It was 10 on one side and 5 on the other. Nobody noticed. Everybody noticed.
- Sidebar "reveal" animation on the right now mirrors the left sidebar's clip behavior instead of freelancing.
- The bar clock right-click opens the control panel (was: same as left-click, sidebar toggle on the whole pill).
- The clock's hover tooltip and the resources hover popup are gone: the dashboards they previewed now appear on hover themselves, and showing both would have stacked two popups on one widget.
- Weather for a day outside the provider's ~7-day window dims the panel and says so, rather than showing stale or blank numbers.
- **Compact sidebar is meaningfully more compact**: padding, rail width, nav/action row heights and grid spacing all trimmed (nav rows 46→41 px, action rows 40→36, grid gaps 5→4, panel padding scale 0.018→0.014). Hit areas stay at 32 px or more.
- **Liquid glass on the small stuff**: bar pills gain a glass rim and sheen clipped to their rounded corners, every `RippleButton` picks up a faint glass rim that brightens on hover, and the sidebar quick-toggles get real glass fills instead of falling through to aurora's transparent circles. New `Appearance.liquid.colEdgeHighlightStrong` token for hovered interactive glass.
- Panel shadows are firmer in real-glass mode (opacity scale 0.55 → 0.85). A translucent panel has no opaque body to separate it from what shows through, so the gentler-than-material shadow that suits an opaque surface is exactly wrong for a see-through one.
- Panel base surfaces route their alpha through `Appearance.panelSurfaceAlpha` instead of hardcoding `applyAlpha(colLayer0, 1)`. That constant is why nothing could ever show through a panel; each surface keeps its own wallpaper-blended tint, only the alpha changes.

### Fixed
- **Hyprland's own colours stopped following the wallpaper.** `defaults/matugen/templates.json` — the manifest that lands in `~/.config/matugen` and that migration 032 syncs over the user's copy — listed only the niri-flavoured consumers. The two Hyprland templates (`hyprland/colors.lua` for the focus ring and window background, `hyprland/hyprlock-colors.conf` for the lock screen) existed only in `dots/.config/matugen`, so the sync replaced a manifest that knew about them with one that did not, and both outputs froze on whatever wallpaper was current at the time. Neither template file shipped in `defaults/` either. Both are now present and registered, alongside the new hyprglass entry.
- **Focus ring, Neovim and fish colours never followed the wallpaper.** The theming templates live in `~/.config/matugen`, outside the shell payload, and were only ever installed by `./setup install` — `./setup update` never re-synced them. Any install that was updated rather than reinstalled therefore kept rendering from the templates present at first install: the `templates.json` manifest never appeared, so the renderer silently fell back to the legacy `config.toml` listing only the original 8 templates, and everything added since (niri focus ring, Neovim, firefox, steam) never rendered at all. The shell itself looked right the whole time because it reads the palette directly. `./setup update` now performs the sync, and migration 032 fixes existing installs. Verified end to end: a red wallpaper now yields a red focus ring gradient and red Neovim accents, a blue one yields blue.
- **fish was never themed at all** — iNiR only ever read `$fish_color_cwd` in a prompt. Added a fish template that renders the full `fish_color_*` / `fish_pager_color_*` set from the palette into `~/.config/fish/conf.d/inir-colors.fish`. The name sorts after fish's own `fish_frozen_theme.fish` (which fish 4.3 wrote when migrating theme vars to global scope) and uses `--global`, so it wins over both that file and any stale universal variables.
- `niri msg action reload-config` no longer exists in niri 26.04 (it is `load-config-file`), so GameMode's animation toggle and the niri visual-refresh migration were silently failing to reload the compositor. Both now try the new name first and fall back to the old one.
- **All-day events spanning two days**: iCal `DTEND` for all-day events is exclusive (RFC 5545) but was treated as inclusive, so every all-day event rendered on its day *and* the next one — dots, day detail, upcoming lists, recurrence expansions. Normalized at parse time; fixed everywhere at once, including multi-day events and recurring ones.
- Calendar cells for trailing next-month days resolved to dates in the *previous* month — clicking "Aug 1" in the July grid opened June 1st. Cells now carry their month offset; bar clock tooltip uses the same shared resolver.
- **The August calendar grid skipped July 31**: the previous/next month-length helpers re-derived lengths with an inverted parity test on the *viewed* month, which read July as 30 days. Every August view showed Jul 26–30 in its leading week instead of Jul 27–31, so those cells opened the wrong day and misplaced event dots. Both helpers now delegate to `getMonthDays` with proper year wrapping; verified against every month of 2024–2028 in both week-start modes.
- `getUpcomingEvents` no longer drops all-day events happening today.
- **GPU stats described the wrong GPU** on systems with more than one DRM card (AMD APU + discrete card): detection took the first `card*` exposing `gpu_busy_percent`, which is typically the APU's ~512 MiB aperture sitting permanently at 0%. The card with the most VRAM is now selected, so the GPU indicator and the new VRAM readout describe the device actually doing the work (measured here: 7% and 4.2/24.0 GB instead of 0% and 0.0/0.5 GB). `boot_vga` is deliberately not used — on desktops the discrete card is usually the boot VGA.
- Calendar month navigation (buttons, wheel, PgUp/PgDn) now slides and fades directionally instead of snap-replacing the grid.

### Notes
- Existing users get the new dashboards automatically — the `knownPanels` migration seeds `iiClockDashboard`/`iiSystemDashboard` into `enabledPanels` on first boot after the update. The clock's old "click for sidebar" habit still works on the empty pill area and the right-edge button.
- The sidebar and dashboard calendars already shared one `CalendarWidget`, so the new press behaviour applies everywhere at once — no second implementation had to be reconciled.
- **niri cannot blur behind a layer surface** — there is no `blur` in its layer-rule vocabulary (verified against niri 26.04). So frosted-glass-over-windows is not available from the compositor, and the two honest options are a blurred wallpaper copy (frosted, but opaque to windows) or genuine alpha (see-through, but not frosted). This release makes the second one the default for liquid and keeps the first available by turning real transparency off.
- The dock is deliberately untouched and still opaque.
