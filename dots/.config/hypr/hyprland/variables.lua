...
-- -n/--no-duplicate keeps a second press from stacking another settings
-- window; a pgrep guard would not work here, since quickshell re-execs and
-- loses the "-p .../settings.qml" command line it was started with.
settingsApp =
	"XDG_CURRENT_DESKTOP=gnome ~/.config/hypr/hyprland/scripts/launch_first_available.sh 'QS_DISABLE_CRASH_HANDLER=1 qs -n -p ~/.config/quickshell/$qsConfig/settings.qml' 'systemsettings' 'gnome-control-center' 'better-control'"
taskManager =
