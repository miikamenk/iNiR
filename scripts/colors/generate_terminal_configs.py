...
def kitty_user_opacity():
    """Read background_opacity from the user's kitty.conf.

    That value is the transparency the user actually asked for. When the herdr
    split (see herdr_pane_split) is active it becomes the *pane* opacity, and
    the generated theme takes over background_opacity itself; kitty.conf stays
    the place where the number is chosen.

    Returns the float, or None when unset/unparseable.
    """
    conf = Path(os.path.expanduser("~/.config/kitty/kitty.conf"))
    if not conf.is_file():
        return None
    value = None
    try:
        for raw in conf.read_text().splitlines():
            ...
            if line.startswith("#"):
                continue
            m = re.match(r"^background_opacity\s+([0-9.]+)\s*$", line)
            if m:
                value = float(m.group(1))  # last one wins, as in kitty
    except (OSError, ValueError):
        return None
    return value


def herdr_pane_split(colors):
    """Decide whether to split kitty's transparency between chrome and panes.

    herdr paints its sidebar by leaving the terminal's default background
    showing, and kitty only applies background_opacity to default-background
    cells — so a translucent terminal makes herdr's chrome translucent too,
    with no herdr-side setting to change it. Giving the panes an explicit
    background (they set it themselves, see write_pane_bg_hooks) moves them out
    of the default-background bucket, which lets the chrome go solid while the
    panes keep the requested opacity.

    Returns (pane_bg, pane_alpha) or None when the split does not apply.
    """
    if not shutil.which("herdr"):
        return None
    alpha = kitty_user_opacity()
    if alpha is None or alpha >= 1.0:
        return None  # opaque terminal: nothing to split
    return colors.get("term0", "#282828"), alpha


def recent_pane_colors(theme_conf, current, limit=7):
    """Current pane color first, then the ones the previous themes used.

    A pane paints its background once, when its shell starts, so panes that
    were already open still carry the previous wallpaper's color. Listing the
    recent ones (kitty allows 7) keeps those panes translucent across a few
    wallpaper changes instead of turning them opaque until the shell restarts.
    """
    colors = [current]
    try:
        for line in Path(theme_conf).read_text().splitlines():
            if line.startswith("transparent_background_colors"):
                for token in line.split()[1:]:
                    color = token.split("@")[0]
                    if re.fullmatch(r"#[0-9A-Fa-f]{6}", color) and color not in colors:
                        colors.append(color)
                break
    except OSError:
        pass
...
def generate_kitty_config(colors, output_path):
    ...
    split = herdr_pane_split(colors)
    if split:
        pane_bg, pane_alpha = split
        # Default background = herdr's panel_bg, so the chrome it leaves
        # unpainted lands on the same color as the tab bar it does paint.
        default_bg = colors.get("background", colors.get("term0", "#282828"))
        pane_colors = recent_pane_colors(
            os.path.join(os.path.dirname(output_path), "theme.conf"), pane_bg
        )
        pane_rule = " ".join(f"{c}@{pane_alpha}" for c in pane_colors)
        transparency = f"""
# herdr transparency split (kitty.conf background_opacity: {pane_alpha})
# Default-background cells — herdr's sidebar and tab bar — stay effectively
# solid; 0.98 rather than 1.0 because kitty only allocates an alpha channel
# below 1, which the pane rule and Hyprland's per-window opacity both need.
# Pane shells paint themselves {pane_bg} (see ~/.config/fish/conf.d/inir-terminal-bg.fish),
# so that color carries the transparency instead. The trailing colors are what
# earlier wallpapers used, so panes opened before the last change stay
# translucent until their shell restarts.
background_opacity 0.98
transparent_background_colors {pane_rule}
"""
    else:
        default_bg = colors.get("term0", "#282828")
        transparency = ""
...
def write_pane_bg_hooks(colors):
    """Give herdr panes their own background color via OSC 11.

    A shell inside a herdr pane talks to herdr's terminal emulator, not to
    kitty, so setting the pane's default background there makes herdr repaint
    that pane with an explicit color when it draws to kitty. kitty then applies
    transparent_background_colors to it, while herdr's chrome — which herdr
    leaves on the terminal default background — stays solid.

    Written unconditionally so a disabled split leaves an inert file behind
    rather than a stale escape sequence.
    """
    home = os.path.expanduser("~")
    split = herdr_pane_split(colors)
    header = "# Auto-generated by ii wallpaper theming system\n# Do not edit manually - changes will be overwritten\n"

    if split:
        pane_bg, pane_alpha = split
        # A kitty window with no herdr in it has no chrome to keep solid, and
        # nothing paints its cells explicitly, so the generated 0.98 would make
        # it near-opaque. Put the requested opacity back on that window.
        fish_body = f"""
# Inside a herdr pane, paint the pane its own background so kitty can keep it
# translucent while herdr's sidebar and tab bar stay solid.
if status is-interactive; and set -q HERDR_ENV
    printf '\\e]11;{pane_bg}\\e\\\\'
else if status is-interactive; and set -q KITTY_LISTEN_ON; and type -q kitten
    kitten @ --to $KITTY_LISTEN_ON set-background-opacity {pane_alpha} >/dev/null 2>&1 &
    disown
end
"""
        posix_body = f"""
...
case $- in
    *i*)
        if [ -n "${{HERDR_ENV:-}}" ]; then
            printf '\\033]11;{pane_bg}\\033\\\\'
        elif [ -n "${{KITTY_LISTEN_ON:-}}" ] && command -v kitten >/dev/null 2>&1; then
            kitten @ --to "$KITTY_LISTEN_ON" set-background-opacity {pane_alpha} >/dev/null 2>&1 &
        fi
        ;;
esac
"""
        ...
        fish_body = "\n# herdr transparency split inactive — nothing to do.\n"
        posix_body = "\n# herdr transparency split inactive — nothing to do.\n"

    fish_path = Path(f"{home}/.config/fish/conf.d/inir-terminal-bg.fish")
    fish_path.parent.mkdir(parents=True, exist_ok=True)
    fish_path.write_text(header + fish_body)

    posix_path = Path(
        f"{home}/.local/state/quickshell/user/generated/terminal/pane-bg.sh"
    )
    posix_path.parent.mkdir(parents=True, exist_ok=True)
    posix_path.write_text(header + posix_body)

    # fish picks conf.d up on its own; zsh and bash need the source line.
    source_line = f'[ -f "{posix_path}" ] && . "{posix_path}"  # iNiR terminal background'
    for rc in (f"{home}/.zshrc", f"{home}/.bashrc"):
        if os.path.isfile(rc):
            ensure_line_in_file(rc, source_line, r"iNiR terminal background")

    print(
        f"✓ Generated herdr pane background hook ({'active' if split else 'inactive'})"
    )
